...
- **Known edge cases / flags to be careful with:**
    ...
    - **Since it _drives_ `sfdx-git-delta` under the hood for incremental deploys**, the same underlying caveats apply transitively (Git LFS objects must already be pulled locally, `HEAD^` vs `HEAD~1` shell-escaping differences on Windows) — see the dedicated `sfdx-git-delta` reference for details.
