...
```
`-d/--dry-run` reports what *would* change without actually submitting any Setup form — the safest way to validate a new/edited `browserforce.json` against an unfamiliar org before committing to it.
